...
router.post("/ai/write", requireAuth, async (req, res): Promise<void> => {
  const { prompt, genre = "fiction", title = "رواية جديدة" } = req.body as {
    prompt?: string;
    genre?: string;
    title?: string;
  };
  ...
  try {
    const stream = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `أنت روائي عربي بارع تكتب روايات طويلة وغنية باللغة العربية الفصحى. أسلوبك أدبي راقٍ يجمع بين وصف عميق للشخصيات والأحداث والمشاعر. اكتب نصاً طويلاً متواصلاً دون عناوين فرعية أو ترقيم.
...
- المحتوى يجب أن يكون لائقاً لجميع الأعمار (PG-13 كحد أقصى).`,
        ...
        {
          role: "user",
          content: `اكتب فصلاً طويلاً وكاملاً لرواية من تصنيف "${genreAr}" بعنوان "${title.trim()}".
